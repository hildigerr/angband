...
/*
 * Something happens to disturb the player.		-CJS- The first arg
...
rest_off()
{
    ...
    py.flags.status &= ~PY_REST;
    prt_state();
    msg_print(NULL);		   /* flush last message, or delete "press
				    * any key" message */
    py.flags.food_digested++;
}
...
int dir, y, x;
{
	...
	if (find_openarea == FALSE) {	/* choose a direction. */
		...
		/*
		 * This corner is seen to be enclosed, and we deliberately go
		 * the long way. 
		 */
		    find_direction = option;
		    find_prevdir = option2;
