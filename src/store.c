...
static s32b sell_price(int snum, s32b *max_sell, s32b *min_sell, inven_type *item)
{
    ...
    /* Get the "basic value" */
    i = i * rgold_adj[owners[s_ptr->owner].owner_race][py.prace] / 100;
...
static void store_prt_gold(void)
{
    ...
    (void)sprintf(out_val, "Gold Remaining : %ld", (long)py.au);
    prt(out_val, 18, 17);
}
...
static int sell_haggle(int store_num, s32b *price, inven_type *item)
{
    ...
    *price = 0;
    ...
    cost = cost * (200 - rgold_adj[o_ptr->owner_race][py.prace]) / 100;
